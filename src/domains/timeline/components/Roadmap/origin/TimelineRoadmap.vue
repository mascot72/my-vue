<template>
  <div class="roadmap-wrapper">
    <VisTimeline
      ...
      :options="options"
      :groups="groups"
      :view-status="{ statusPlan, statusExec, statusNBiz }"
      :view-mode="viewMode"
      :hide-empty-groups="hideEmptyGroups"
      @open-detail-slide="onDetailView"
    ...
    <div class="roadmap-tree">
      <div class="roadmap-tree-row">
        <div class="flex align-items-center horizontal">
          <span class="roadmap-tree-title"
            ><dxplm-label message="tes.roadmap-tree-label" text="로드맵 Tree"
          /></span>
          <dxplm-icon-button
            type="line"
            icon="collapse_menu"
        ...
        <dxplm-button
          v-if="flag % 3 === 0"
          small
          class="view-mode step-1"
          @click="changeItemMode(false)"
        >
        ...
        <dxplm-button
          v-else-if="flag % 3 === 1"
          small
          class="view-mode step-2"
          @click="changeItemMode(false)"
        >
          <dxplm-label
            message="tes.tech-class-system-label"
            text="기술분류체계"
          />
        </dxplm-button>
        <dxplm-button
          v-else
          small
          class="view-mode step-3"
          @click="changeItemMode(false)"
        >
          <dxplm-label
            message="tes.common-tech-types-label"
            text="공통기술유형"
          />
        </dxplm-button>
      </div>
    ...
    <div class="roadmap-toolbar">
      ...
      <ul class="roadmap-toolbar-lists tool">
        ...
        >
          <dxplm-icon-button
            type="line"
            :icon="item.icon"
            @click="handleClick(index, item.handler)"
          />
      ...
      <ul class="roadmap-toolbar-lists">
        ...
        <li class="roadmap-toolbar-list today">
          ...
          <dxplm-tooltip hover left
            ><dxplm-label message="cmm.en-today-button" text="Today"
          /></dxplm-tooltip>
        </li>
        <!-- 월/분기 버튼 -->
        <li
      ...
      <ul class="roadmap-toolbar-lists zoom">
        <li
          v-for="(item, index) in zoomItems"
          :key="index"
          class="roadmap-toolbar-list"
        >
          <dxplm-icon-button
            type="line"
            :icon="item.icon"
            @click="handleZoom(item.action)"
          />
          <dxplm-tooltip hover left>{{ item.label }}</dxplm-tooltip>
        </li>
      </ul>
...
function viewPjt() {
  ...
  console.log("과제실행 현황조회");
}
function reviewCross() {
  console.log("Cross BU 중복기술 검토");
}
