...
import VisTimelineGroup from "./VisTimelineGroup.vue";
import VisTimelineItem from "./VisTimelineItem.vue";
 ...
 template: function (item, element, data) {
  ...
  render(
   h(VisTimelineItem, {
    ...
    onAddClick: (itemId) => {
     // VisTimelineItem의 addClick 이벤트로 연결
     visibleSubTechTree(
...
const toggleAllGroups = (show) => {
 ...
 if (updatesMap.size > 0) {
  groups.update(Array.from(updatesMap.values()));
 }
};
...
const toggleNestedGroups = (parentId, show) => {
 ...
 groups.update({ id: parentId, showNested: show });
...
const reloadGroups = (newGroups, initVisibleGroup = true) => {
 ...
 if (initVisibleGroup) {
  ...
  const sanitizedGroups = toRaw(newGroups).map((g) => ({
   ...g,
...
const reloadItems = (newAllItems) => {
 if (!newAllItems || !newAllItems.length > 0) return;
 const filtered = newAllItems.filter((i) => visibleGroups.value.has(i.group));
 timeline.value?.setItems(new DataSet(toRaw(filtered)));
};
...
onMounted(() => {
 ...
 groups = new DataSet(sanitizedGroups);
 ...
 // 툴팁 이벤트 핸들링
 timeline.value.on("itemover", (eventProps) => {
