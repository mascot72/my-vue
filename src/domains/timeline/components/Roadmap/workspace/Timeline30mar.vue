...
<!-- 이 원본에서 하나씩 분할해서 점진적으로 작성한다! (3/30) -->
<template>
...
import TimelineArrows from "@/modules/tes/ermm/components/visTimelineArrows.js";
import ItemInfoPopup from "@/modules/tes/ermm/components/ErmmItemInfoPopup.vue";
import VisContextMenu from "@/modules/tes/ermm/components/ErmmVisContextMenu.vue";
...
import { useTimeline } from "@/modules/tes/ermm/components/timeline/useTimeline";
...
import {
  ...
  syncDataSetById,
} from "@/modules/tes/ermm/components/timelineTemplateInjector";
...
const reloadData = async () => {
  if (isLoading || !timeline.value) return; // 이미 실행 중이면 차단
  ...
  console.log("reloadindData error:", e);
 } finally {
  isLoading = false;
 }
};
const debouncedReload = debounce(reloadData, 200);
...
const { handleItemOver } = useTimeline(props, emit, store, getDdName, getTechClassificationName);
...
onMounted(async () => {
  ...
  // store에 함수 주입(axios useHttp의 한계로 인해서 필요함!)
  store.setTechNameFn(getTechClassificationName);
  ...
  timeline.value.on("itemover", async (eventProps) => {
    ...
    /** sub techs Lazy Load Logic 처리 */
    handleItemOver(itemId);
