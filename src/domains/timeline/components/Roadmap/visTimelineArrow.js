...
export default class VisTimelineArrows {
  /**
   * Creates arrows.
  ...
  _initialize() {
    ...
    for (const element of this._dependency) {
      this._createPath(element.color, element.line);
    }
   ...
   * @private
   * @param {String} color arrow color
   * @param {Number} lineType arrow lineType
   * */
  _createPath(color, lineType) {
    ...
    // Set the line type
    const line = lineType === undefined ? 0 : lineType; // Default to solid line
    if (line === 0) {
  ...
  /** @private */
  _drawDependencies() {
    const firstItemIds = this.getActiveFirstItemIds();
    for (const firstItemId of firstItemIds) {
      this._drawDependenciesByFirstItemId(firstItemId);
    }
  }

  _drawDependenciesByFirstItemId(firstItemId) {
    let targetDependency = this._dependency.filter((dep) => dep.id_item_1 === firstItemId);
    let minX = null;
    for (const element of targetDependency) {
      let item_1 = this._getItemPos(
        this._timeline.itemSet.items[element.id_item_1],
      );
      let item_2 = this._getItemPos(
        this._timeline.itemSet.items[element.id_item_2],
      );
      if (!item_1.isVisible && !item_2.isVisible) continue;
      const min = Math.min(
        item_1.isVisible ? item_1.left : item_2.left,
        item_2.isVisible ? item_2.left : item_1.left,
      );
    ...
    //Create paths for the started dependency array
    for (let i = 0; i < targetDependency.length; i++) {
      this._drawArrows(targetDependency[i], minX);
    }
  }
   ...
   * @param {number} minX arrow minX
   */
  _drawArrows(dep, minX) {
    ...
    const index = this._dependency.indexOf(dep);

    const bothItemsExist =
      this._timeline.itemsData.get(dep.id_item_1) !== null &&
      this._timeline.itemsData.get(dep.id_item_2) !== null;
    ...
    if (bothItemsExist) {
      ...
      for (const element of visibleItems) {
        if (dep.id_item_1 === element) oneItemVisible = true;
        if (dep.id_item_2 === element) oneItemVisible = true;
      }

      if (this._timeline.groupsData) {
        ...
        if (this._timeline.groupsData.get(groupOf_1)) {
          groupOf_1_isVisible = true;
        }

        if (this._timeline.groupsData.get(groupOf_2)) {
    ...
    if (
      groupOf_1_isVisible &&
      groupOf_2_isVisible &&
      (oneItemVisible || !this._hideWhenItemsNotVisible) &&
      bothItemsExist
    ) {
      let item_1 = this._getItemPos(
        this._timeline.itemSet.items[dep.id_item_1],
      );
      let item_2 = this._getItemPos(
        this._timeline.itemSet.items[dep.id_item_2],
      );

      // For arrows without special parameters (type: 0, no align), return the old behavior
      const arrowType = dep.type === undefined ? 0 : dep.type;
      ...
      if (
        ...
        item_2.mid_x < item_1.mid_x
      ) {
        [item_1, item_2] = [item_2, item_1]; // Swap for standard arrows
        swapped = true;
      }

      let curveLen = item_1.height * 2;
      const markerId = this._getOrCreateMarker(dep.color);
      const direction = dep.direction === undefined ? 1 : dep.direction;
      let markerStart = "";
      let markerEnd = "";
      ...
      if (arrowType === 1) {
        ...
        const getRectIntersection = (rect, x0, y0, x1, y1, offset = 0) => {
          ...
          if (dx !== 0) {
            ...
            if (t > 0 && t < 1) {
              ...
              if (y >= rect.top && y <= rect.bottom)
                candidates.push({ x: rect.left, y, t });
            }
          }
          // Right side
          if (dx !== 0) {
            ...
            if (t > 0 && t < 1) {
              const y = y0 + t * dy;
              if (y >= rect.top && y <= rect.bottom)
                candidates.push({ x: rect.right, y, t });
            }
          }
          // Top side
          if (dy !== 0) {
            ...
            if (t > 0 && t < 1) {
              ...
              if (x >= rect.left && x <= rect.right)
                candidates.push({ x, y: rect.top, t });
            }
          }
          // Bottom side
          if (dy !== 0) {
            ...
            if (t > 0 && t < 1) {
              const x = x0 + t * dx;
              if (x >= rect.left && x <= rect.right)
                candidates.push({ x, y: rect.bottom, t });
            }
          }
          ...
          candidates.sort(
            (a, b) =>
              Math.hypot(a.x - x0, a.y - y0) - Math.hypot(b.x - x0, b.y - y0),
          );
          let pt = candidates[0];
        ...
        if (dep.align === "left") {
          ...
          // const xA = Math.min(start.x, end.x) - offset;
          const xA = (minX ? minX : Math.min(start.x, end.x)) - offset;

          if (direction === 1 || direction === 3) end.x -= 10;
          if (direction === 2 || direction === 3) start.x -= 10;
          ...
          const overlap =
            item_1.left < item_2.right && item_1.right > item_2.left;
  ...
  _getItemPos(item) {
    ...
    if (this._timeline.options.orientation.item == "top") {
      top_y = (item.parent?.top || 0) + (item.top || 0);
    } else {
      top_y = item.parent.top + item.parent.height - item.top - item.height;
    }

    const container = this._timeline.body.dom.center;
    ...
    const isVisible =
      item.displayed &&
      left_x + item.width >= scrollLeft &&
      left_x <= scrollLeft + clientWidth &&
      top_y + item.height >= scrollTop &&
      top_y <= scrollTop + clientHeight;
  ...
  /**
   * Get unique id_item_1 from dependencies.
   * @returns {VisIdType[]} list of unique item 1 ids
   */
  getActiveFirstItemIds() {
    return [...new Set(this._dependency.map((dep) => dep.id_item_1))];
  }
  ...
  removeArrow(id) {
    ...
    if (index >= 0) {
      let list = document.getElementsByTagName("path"); //FALTA QUE ESTA SELECCION LA HAGA PARA EL DOM DEL TIMELINE INSTANCIADO!!!!
   ...
   * Finds arrow with the given id and removes it.
   * Función que recibe el id de una flecha y la elimina.
   * @param {ArrowIdType} idItem1 arrow id_item_1
   */
  removeArrows(idItem1) {
    for (let i = this._dependency.length - 1; i >= 0; i--) {
      if (this._dependency[i].id_item_1 === idItem1) {
        const el = this._dependencyPath[i];
        if (el && el.parentNode) {
          el.parentNode.removeChild(el);
        }
        this._dependency.splice(i, 1);
        this._dependencyPath.splice(i, 1);
      }
    }
  }
  removeArrowAll() {
  ...
  removeItemArrows(id) {
    ...
    for (let i = 0; i < this._dependency.length; i++) {
      if (
        this._dependency[i].id_item_1 == id ||
        this._dependency[i].id_item_2 == id
      ) {
  ...
  removeArrowsBetweenItems(itemId1, itemId2) {
    let listOfRemovedArrows = [];
    let ArrowsToDelete = this._dependency.filter(
      (dep) => dep.id_item_1 == itemId1 && dep.id_item_2 == itemId2,
    );
